...
import net.nyko.common.ModBeaconTileEntity.BeamSegment;

@OnlyIn(Dist.CLIENT)
public class ModBeaconTileEntityRender extends TileEntityRenderer<ModBeaconTileEntity> {

	public static final ResourceLocation TEXTURE_BEACON_BEAM = new ResourceLocation("textures/entity/beacon_beam.png");

	public ModBeaconTileEntityRender(TileEntityRendererDispatcher rendererDispatcherIn) {
	...
	public void render(ModBeaconTileEntity tileEntityIn, float partialTicks, MatrixStack matrixStackIn, 
		...
		List<BeamSegment> list = tileEntityIn.getBeamSegments();
		int j = 0;

		for (int k = 0; k < list.size(); ++k) {
			
			BeamSegment beacontileentity$beamsegment = list.get(k);
			renderBeamSegment(matrixStackIn, bufferIn, partialTicks, i, j, k == list.size() - 1 ? 1024 : beacontileentity$beamsegment.getHeight(),
	...
	public static void renderBeamSegment(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn,
			ResourceLocation textureLocation, float partialTicks, float textureScale, long totalWorldTime, int yOffset,
		...
		renderPart(matrixStackIn, bufferIn.getBuffer(RenderType.getBeaconBeam(textureLocation, false)), f3, f4, f5,
		...
		renderPart(matrixStackIn, bufferIn.getBuffer(RenderType.getBeaconBeam(textureLocation, true)), f3, f4, f5,
	...
	private static void addQuad(Matrix4f matrixPos, Matrix3f matrixNormal, IVertexBuilder bufferIn, float red,
			float green, float blue, float alpha, int yMin, int yMax, float x1, float z1, float x2, float z2, float u1,
			float u2, float v1, float v2) {
	...
	private static void addVertex(Matrix4f matrixPos, Matrix3f matrixNormal, IVertexBuilder bufferIn, float red,
			float green, float blue, float alpha, int y, float x, float z, float texU, float texV) {
		
		bufferIn.pos(matrixPos, x, (float) y, z).color(red, green, blue, alpha).tex(texU, texV)
